...
// Force HTTP/WS to avoid localhost issues
...
const ElectionDashboard = () => {
  ...
  // 1. Fetch Data Function (Reusable & Memoized)
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (distance < 0) {
        ...
        // ADDED: Calculate Days
        const days = Math.floor(distance / (1000 * 60 * 60 * 24));
        
        // Existing calculations (Hours is correct as the "remainder" only if Days is shown alongside it)
        const hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
        const minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
        const seconds = Math.floor((distance % (1000 * 60)) / 1000);
        
        // CONDITIONAL DISPLAY: Show days if > 0
        if (days > 0) {
    ...
    <Layout>
      <div className="px-6 lg:px-20 py-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-end gap-6 mb-8">
            ...
            {/* RIGHT: Compact Stats (Votes & Turnout) */}
                        ...
                        currentPosition.candidates.map((candidate) => (
                            <div key={candidate.id} className="relative group p-5 rounded-xl bg-[#0a0a18] border border-[#9D4EDD]/20 hover:border-[#00E5FF] transition-all overflow-hidden">
                                ...
                                <div className="relative z-10 flex items-center gap-5">
                                    {/* STATIC AVATAR (No longer clickable) */}
